...
function EcoindexBadge(lang) {
  ...
  **/
  ...
  const ecoindex = (dom, resources) => {
    ...
    let size = 0;
    let req = 0;
    let error = 0; // Parcours les ressources pour le nombre de requette et leur poids

    resources.forEach(function (resource) {
      // Nombre de ressource
      req++; // Poids du fichier en octets typeof resource.transferSize !== 'undefined' &&

      let size_file = 0; // Si la boucle n'arrive pas à lire le poids du fichier on ajoute un % d'erreur
    ...
    let dic = lang.lang.toString().toUpperCase() === 'EN' ? dictionary.EN : dictionary.FR; // Affichage dans la barre d'admin

    let ecotitle = 'ecoIndex: ' + ecoIndex.toFixed(2) + (p100error > 0 ? ' (*' + Math.round(p100error) + dic.percentError + ')' : '') + ' | ' + dic.ges + ': ' + ges + ' ' + dic.gCO2e + ' | ' + dic.water + ': ' + eau + ' cl | ' + dic.reqNum + ': ' + req + ' | ' + dic.pageSize + ': ' + size + ' Ko | ' + dic.domSize + ': ' + dom; // Style de la gélule d'affichage d'ecoindex
    ...
    style.sheet.insertRule('#ecoindex span.G { background-color: #ED2124; color: #fff; }'); // Ajout de la note dans la barre d'admin

    setComponent(<a href={dic.pageURL} id="ecoindex" target="_blank" title={ecotitle}>
        ecoIndex
